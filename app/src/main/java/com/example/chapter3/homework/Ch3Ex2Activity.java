...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator ex2_1_1 = ObjectAnimator.ofFloat(target, "scaleX", 1.0f, 2.0f);
        ex2_1_1.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        ex2_1_1.setRepeatCount(ObjectAnimator.INFINITE);
        ex2_1_1.setRepeatMode(ObjectAnimator.REVERSE);
        ObjectAnimator ex2_1_2 = ObjectAnimator.ofFloat(target, "scaleY", 1.0f, 2.0f);
        ex2_1_2.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        ex2_1_2.setRepeatCount(ObjectAnimator.INFINITE);
        ex2_1_2.setRepeatMode(ObjectAnimator.REVERSE);

        // TODO ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator ex2_2 = ObjectAnimator.ofFloat(target, "alpha", 1.0f, 0.5f);
        ex2_2.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        ex2_2.setRepeatCount(ObjectAnimator.INFINITE);
        ex2_2.setRepeatMode(ObjectAnimator.REVERSE);

        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1, ex2_1_1, ex2_1_2, ex2_2);
        animatorSet.start();
    }
}
